...
class SudokuGame:
        ...
        for i in range(1, 10):
            for j in range(1, 10):
                ...
                if cell_value != 0:
                    ...
                    entry = SudokuEntry(self.master, self, row=i-1, col=j-1)
    ...
    def validate_entry(self, row, col, value):
        if value.isdigit() and 1 <= int(value) <= 9:
            if self.is_valid_move(self.board, row, col, int(value)):
                ...
                self.update_entry(row, col, "")
        else:
            self.update_entry(row, col, "")

    def update_entry(self, row, col, value):
        widget_names = self.master.children.keys()
        print("Available widget names:", widget_names)

        entry_name = f'!sudokuentry{row}.{col}'
        entry = self.master.nametowidget(entry_name)
        print(f"Trying to access widget: {entry_name}")
    ...
    def solve_puzzle(self):
        for i in range(9):
            for j in range(9):
                if self.master.nametowidget(f'!sudokuentry{i}.{j}').get() == "":
                    self.update_entry(i, j, str(self.initial_board[i][j]))
...
class SudokuEntry(tk.Entry):
    def __init__(self, master, game, row, col, **kwargs):
        super().__init__(master, width=2, font=('Helvetica', 16), justify='center', **kwargs)
        self.bind('<KeyRelease>', lambda event, r=row, c=col: game.validate_entry(r, c, self.get()))
